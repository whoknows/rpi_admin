...
(function(B) {
	...
	B.color.parse = function(F) {
		...
		if (E = /rgba\(\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\s*\)/.exec(F))
			return C(parseFloat(E[1]) *
					2.55, parseFloat(E[2]) * 2.55, parseFloat(E[3]) * 2.55, parseFloat(E[4]));
	...
	var A = {aqua: [0, 255, 255], azure: [240, 255, 255], beige: [245,
...
(function($) {
	function Plot(placeholder, data_, options_, plugins) {
		var series = [], options = {colors: ["#edc240", "#afd8f8", "#cb4b4b", "#4da74d", "#9440ed"], legend: {show: true, noColumns: 1, labelFormatter: null, labelBoxBorderColor: "#ccc", container: null, position: "ne", margin: 5, backgroundColor: null, backgroundOpacity: 0.85}, xaxis: {show: null, position: "bottom", mode: null, color: null, tickColor: null, transform: null, inverseTransform: null, min: null, max: null, autoscaleMargin: null, ticks: null, tickFormatter: null, labelWidth: null, labelHeight: null,
			...
			processDatapoints: [], drawSeries: [], draw: [], bindEvents: [], drawOverlay: [], shutdown: []}, plot = this;
		...
		plot.pointOffset = function(point) {
			return{left: parseInt(xaxes[axisNumber(point,
						"x") - 1].p2c(+point.x) + plotOffset.left), top: parseInt(yaxes[axisNumber(point, "y") - 1].p2c(+point.y) + plotOffset.top)}
		};
		...
		function parseOptions(opts) {
			...
			if (options.points)
				$.extend(true,
						options.series.points, options.points);
			if (options.bars)
		...
		function fillInSeriesOptions() {
			var i;
			var neededColors = series.length, usedColors =
					[], assignedColors = [];
			...
			while (colors.length < neededColors) {
				...
				c.scale("rgb",
						1 + sign * Math.ceil(variation / 2) * 0.2);
				colors.push(c);
				++i;
		...
		function processData() {
			var topSentry =
					Number.POSITIVE_INFINITY, bottomSentry = Number.NEGATIVE_INFINITY, fakeInfinity = Number.MAX_VALUE, i, j, k, m, length, s, points, ps, x, y, axis, val, f, p;
			...
			for (i =
				...
				for (j = k = 0; j < data.length; ++j, k += ps) {
					...
					else if (insertSteps && k > 0 && points[k - ps] != null && points[k - ps] != points[k] && points[k - ps + 1] != points[k + 1]) {
						for (m = 0; m < ps; ++m)
			...
			for (i = 0; i < series.length; ++i) {
				...
				var xmin = topSentry,
						ymin = topSentry, xmax = bottomSentry, ymax = bottomSentry;
		...
		function allocateAxisBoxFirstPhase(axis) {
			...
			if (axis.direction == "x") {
				...
				if (pos == "bottom") {
					...
					axis.box =
							{top: plotOffset.top + axismargin, height: lh};
		...
		function setupTickGeneration(axis) {
			...
			var delta = (axis.max - axis.min) / noTicks, size, generator, unit, formatter, i, magn,
					norm;
			if (opts.mode == "time") {
				var timeUnitSize = {"second": 1E3, "minute": 60 * 1E3, "hour": 60 * 60 * 1E3, "day": 24 * 60 * 60 * 1E3, "month": 30 * 24 * 60 * 60 * 1E3, "year": 365.2425 * 24 * 60 * 60 * 1E3};
		...
		function draw() {
			...
			for (var i = 0; i < series.length; ++i) {
				executeHooks(hooks.drawSeries, [ctx,
					series[i]]);
				drawSeries(series[i])
			}
		...
		function insertAxisLabels() {
			...
			for (var j = 0; j < axes.length; ++j) {
				...
				if (!axis.show)
					continue;
				html.push('<div class="' + axis.direction + "Axis " + axis.direction + axis.n + 'Axis" style="color:' +
						axis.options.color + '">');
		...
		function drawSeriesLines(series) {
			...
			if (lw > 0 && sw > 0) {
				...
				plotLine(series.datapoints, Math.sin(angle) * (lw / 2 + sw / 2), Math.cos(angle) * (lw / 2 + sw / 2), series.xaxis,
						series.yaxis);
				ctx.lineWidth = sw / 2;
		...
		function drawSeriesPoints(series) {
			function plotPoints(datapoints, radius,
					fillStyle, offset, shadow, axisx, axisy, symbol) {
		...
		function getFillStyle(filloptions, seriesColor, bottom, top) {
			...
			if (filloptions.fillColor)
				return getColorOrGradient(filloptions.fillColor,
		...
		function findNearbyItem(mouseX, mouseY, seriesFilter) {
			var maxDistance =
					options.grid.mouseActiveRadius, smallestDistance = maxDistance * maxDistance + 1, item = null, foundPoint = false, i, j;
				...
				if (s.bars.show && !item) {
					...
					for (j = 0; j < points.length; j += ps) {
						...
						if (series[i].bars.horizontal ?
								mx <= Math.max(b, x) && mx >= Math.min(b, x) && my >= y + barLeft && my <= y + barRight : mx >= x + barLeft && mx <= x + barRight && my >= Math.min(b, y) && my <= Math.max(b, y))
							item = [i, j / ps]
					}
				}
			}
